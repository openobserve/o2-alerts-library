...
See [`PACKS.md`](../../../PACKS.md) for the current alert and category counts.
They are generated by CI from the tree on every merge — the hand-written table
that used to sit here had drifted to 76/12 against a real 86/14, missing
`cert-manager` and `other` entirely, which is why it is gone.
